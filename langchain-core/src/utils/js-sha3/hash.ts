...
// Inlined SHA3 implementation for portability (node, browser, etc.)
...
var createCshakeOutputMethod = function (bits, padding, outputType) {
  return function (message, outputBits, n, s) {
    return methods["cshake" + bits]
      .create(n, s)
      .update(message)[outputType](outputBits);
  };
};
...
var createCshakeMethod = function (bits, padding) {
  ...
  method.create = function (n, s) {
    if (!n && !s) {
      return methods["shake" + bits].create();
    } else {
      return new Keccak(bits, padding, null).bytepad([n, s], w);
    }
  };
  method.update = function (message, outputBits, n, s) {
    return method.create(n, s).update(message);
  };
    ...
    if (type === "object") {
      if (str === null) {
        ...
        if (!ArrayBuffer.isView(str)) {
          throw new Error("str must be string or array");
        }
      }
    } else {
      throw new Error("str must be string or array");
...
// Export specific SHA3-256 function for our use case
export const secureHash = (message: string): string => {
  return methods.sha3_256(message);
};